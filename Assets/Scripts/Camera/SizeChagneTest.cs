...
public class SizeChagneTest : MonoBehaviour
{
    ...
    void Update()
    {
        if (isTransition || isReturn)
        {
            ...
            if (isTransition)
            {
                virtualCamera.m_Lens.OrthographicSize = Mathf.Lerp(initialSize, targetSize, process);
            }
            else
            {
                virtualCamera.m_Lens.OrthographicSize = Mathf.Lerp(targetSize, initialSize, process);
            }
